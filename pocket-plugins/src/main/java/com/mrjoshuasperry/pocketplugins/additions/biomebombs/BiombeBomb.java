...
public class BiombeBomb extends Module {
  ...
  public BiombeBomb() {
    super("biomebombs");
    plugin = PocketPlugins.getInstance();
    biomeBombTypeKey = new NamespacedKey(plugin, "biomb_bomb_type");
    biomeBombColorKey = new NamespacedKey(plugin, "biomb_bomb_color");
  ...
  public void init(YamlConfiguration config) {
    ...
    this.plugin.getCommand("biomebombs").setExecutor(new BiomeBombCommand(biomeBombsData));
  }
